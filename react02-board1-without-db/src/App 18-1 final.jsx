...
const GlobalTop = (props) => {
  ...
  */
  ...
  //이 컴포넌트가 1차 랜더링된 후 실행되는 훅 정의
  useEffect( function(){
    console.log("#Life", "LifeGood ===> 3. useEffect 실행1");
    //JSON 가져오기
    /* 내부에 있는 json파일을 get 방식으로 요청한다. */
    fetch("./json/myData.json")
    .then( ( result ) => {
      /* 요청에 성공하면 json파일의 데이터가 매개변수를 통해 반환된다.
        반환된  데이터는 Text형식이므로 JSON포맷으로 변환후 반환한다..
      */
      return result.json();
    })
    .then( ( json ) => {
      /** 첫번째 then절에서 반환한 값은 두번째 then절로 반환된다
       * 이값을 받은 후 데이터로 사용할 스테이트를 변경한다.
      */
      console.log(json);
      setMyList(json);
    });
    return() => {
      /* 컴포넌트가 언마운트될때 실행할 코드가 있다면 이부분에 작성하면 된다.
      이 부분은 두번째 랜더링부터 동작한다. */
  ...
  /* 
    useEffect의 두번째 인자인 의존성배열에 빈 배열을 추가한다
     ...
     만약 의존성배열을 생략하면 무한히 로딩되는 현상이 발생된다<div className="use"></div>
     useEffect 배부에서 스테이트를 변경하는 코드가 있기 때문이다.
  */
  ...
  // 각 루프에 해당하는 객체를 인출
    ...
    listTag.push(
      <li key={data.id}>
        {/* data-id 속성에 설정된 값은 Event 객체의 target속성 하위의
...
function App(){       
  ...
  return (<>
      ...
      <div className="App">
        ...
        자식 컴포넌트는 이 함수를 호출할때 게시물의 일련번호르 인수로
        전달한다. 
        */}
        <GlobalTop myLinkClick = { (num) => {
          ...
          fetch( "./json/dto"+num+".json")
            ...
            .then( (json ) => {
                console.log("결과",json);
                //JSON파일의 내용을 통해 스테이트를 변경하고 리렌더링 한다.
                setMyResult( json );
            });
        }}></GlobalTop>
