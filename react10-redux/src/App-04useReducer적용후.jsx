...
/*
  ...
  1. props라는 대표 매개변수를 사용한다. 이때는 2개이상의 인술르 객체형태로
  받을수 있으므로 'props.속성명'과 같이 사용한다
  2. 인수를 개별 변수로 전달받는다
{ 매개변수1, 매개변수2....N }
...
const Student = ( { name, dispatch, id, isHere  } ) => {
  return (<>
    <div>
      {/*  학생이 이름을 클릭하면 출석 기능이 토글 */}
      {/* 학생이 출석한 상태이면 가운데줄과 회색으로 변경되므로 스타일
      속성을 추가함 이상태는 isHere에 따라 토글된다.*/}
      <span style={{ 
...
const reducer = ( state, action ) => {
  ...
  switch( action.type ){
    //학생 추가
    case 'add':
        //학생이름을 액션객체를 통해 읽어온다. dispatch 함수내에 객체에서 얻어온다.
        const name = action.param.name;
        //추가를 위한 새로운 객체 생성
        const newStudent = {
          id: Date.now(),
          name, /* 이름은 key와 value가 동일 하므로 하나만 작성 */
          isHere : false, /* 출석여부 */
        }
        return{
          //학생수 증가
    ...
    case 'delete' :
      // console.log("1111");
      // //const count = action.param.count;
      // const id = action.param.id;
      // console.log( "아이디 : " + id );
      // let newStudnet2 = [];    

      // for( let i = 0; i < state.students.length; i++  ){
      //   console.log('re 아이디 ' + state.students[i].id );  
      //     if( state.students[i].id !== id ){
      //       console.log("들어온다.")
      //       newStudnet2.push(state.students[i]);
      //     }
      // }
      // console.log(" 777 "+newStudnet2);
       ...
       return{
        //--students : newStudent2
          ...
          /*filter함수를 이용해서 삭제할 학생을 제외한 나머지 객체를 
           반환하도록 정의한 후 반환값을 통해 업데이틑한다.
           화살표 함수는 매개변수가 1개인 경우 소괄호 생략이 가능ㅎ라고,
    ...
    case 'mark' :
      return {
        //카운트는 변함 없음
        count : state.count,
        /* 출결처리는 학생수에는 변함이 없으므로 map을 사용한다<div className=""></div>
          처리할 학생을 찾은후  isHere 부분만 true/false로 변경한다.
