...
/*
  ...
  컴포넌트 내부에서 발생하는 데이터를 가져오기, 구독설정, 수동으로
  DOM조작등과 같은 작업을 수행한다.
  컴포넌트가 1차 랜더링을 마친후 실행할 코드를 정의할때 사용된다
...
function LifeGood( props ){
  /*
    이 컴포넌트에서 제일 먼저 실행되는 코드. 즉 랜더링 전에 실행할
    코드가 있다면 이 부분에 작성한다.
  */
  console.log( '#Life', 'LifeGood ==>1. 컴포넌트 실행(함수 호출)' );

  //스테이트 생성 : 컴포넌트 스테이트가 변경될때마다 새롭게 랜더링된다.
  //프롭스로 전달된 값으로 초기화
  ...
  * */
  ...
  /**
    ...
    1. 2개의 버튼ㅇ르 누를때마다 useEffect가 실행된다. 즉 리렌더링이
    될때마다 실행된다<div className="1."></div>
    2. 최초 실행시에만 useEffect가 실행되고, 그 이후에는 실행되지 않는다<div className=""></div>
    3. myCount가 변경될때만 useEffect각 실행된다.
   */
