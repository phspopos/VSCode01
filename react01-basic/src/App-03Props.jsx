...
/*
  ...
  전달하기 위해 읽기 전용 데이터르 말한다. 전달시에는 HTML의 속성처럼 기술한다.
  형시]
  <컴포넌트 props속성명 = {전달할값} />
...
/* 
  컴포넌트로 전달되는 모든 프롭스를 매개 변수 props를 통해 한꺼번에 받는다.
  이렇게 받은 객체와 같이 'props.프롭스명'으로 사용 할 수 있다.
*/
function FromtComp( props ){
  ...
  /* 앞에서<li>태그가 추가된 배열변수를 UI에 삽입한다.  이부분에
  있는 내용이 화면에 랜더링된다.*/
  return (<>
...
/*
  React에서는 게시판의 ㅇ목록과 같이  반복적으로 출력되는 함목에
  unique한 key라는 이름의 props을 추가하도록 권고하고 있다<div className="ㄸ"></div>
...
function App(){
  ...
  //props로 사용할 배열 변수 선언
  const fontData = ['HTML5', 'CSS3', 'Javascript', 'jQuery', 'React추가' ];
  ...
  return (<>
    <div>
      ...
      <ol>
          ...
          <FromtComp propData1 = {fontData} aTemp={100}></FromtComp>
          <BackComp propData2 = {backData} bTemp={200} />
      </ol>
