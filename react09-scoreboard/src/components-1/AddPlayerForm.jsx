...
/*
  일반적으로 컴포넌트 선언과 export default 는 따로 진행하지만
  ...
  return (<>
    <form className="form" noValidate 
      onSubmit={ (e) => {
        ...
        //방법2 : useRef 훅 사용                
        //useRef 를 통해 input요소의 현재 값에 접근
        const playName2 = playerInputRef.current.value;
        if( playName2 == '' ){
      ...
      {/* 위에서 생성한 ref변수를 추가한다. ref={ playerInputRef } */}
      <input type="text" name="player" minLength="10" className="input" 
        placeholder="이름을 추가하세요" required onChange={()=>{}}  ref={ playerInputRef }/>
        { /* 선수추가 경고창은 제거합니다. */}
