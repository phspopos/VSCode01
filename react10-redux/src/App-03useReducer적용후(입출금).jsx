...
const myReducer = ( nowState, myAction ) => {
  ...
  //Action을 분석한 후 입출금 및 잔고조회를 처리하다.
  switch( myAction.mode ){
    case ActionType.depo : 
...
function App() {
  ...
  return (<>
    ...
    <div className="App">
      ...
      {/* 앞에서 선언한 상수를 이용해서 Action객체를 완성환 후
      디스페치 함수의 인수로 전달한다. */}
      <button type='button' onClick={ () => {
        myDispatch( { mode:ActionType.depo, amount:number } );
      }}>입금</button>
      <button type='button' onClick={ () => {
        myDispatch( { mode:ActionType.with, amount:number });
      }}>출금</button>
