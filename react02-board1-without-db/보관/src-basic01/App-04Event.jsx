...
/*
  이벤트 처리
   ...
   하지마 React는 이벤트명의 첫글자를 반드시 대문자로 기술해애한다
...
function MyBody( props ){
  ...
  return(<>
    <ol>
      {/* 첫번째 경과창은 고정된 메세지를 알림창으로 출력한다
      props로 전달된 함수를 자식 컴포넌트에서 그대로 사용하는 형식이다
      링크를 클릭하면 알림창이 뜨고, 닫으면 화면이 새로고침 된다.*/}
      <li><a href="/" onClick={()=>{ props.onMyAlert1(); }}>프론트앤드</a></li>
      <ul>
          {liTag1}
      </ul>
      {/* 이벤트 객체를 통해 화면이 새로고침 되지않도록 요청을 차단한다<div className="R"></div>
      React는 비동기 방식으로 화면을  전환하므로 화면이 새로고침 되면 안된다<div className="이"></div>
